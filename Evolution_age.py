...
matrice = np.array([[0.6,0.35,0.5,1],
                    [0.4,0,0,0],
                    [0,0.65,0,0],
                    [0,0,0.5,0]])
...
matrice2 = [[0.73, 1.25, 1.25, 1.25, 0],
             [0.39, 0, 0, 0, 0],
             [0, 0.67, 0, 0, 0],
             [0, 0, 0.67, 0, 0],
             [0, 0, 0, 0.64, 0]]
...
def graph_with_animation1(matrice,vector):

    #on défini notre graphe ou chaque ligne correspond à un vecteur pour l'instant vide
    fig, ax = plt.subplots()
    ligne, = ax.plot([], [], 'o-', label='a0')
    ligne1, = ax.plot([], [], 'v-', label='a1')
...
graph_with_animation1(matrice,vector)
